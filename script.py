...
for name, ticker in targets:
    try:
        ...
        # ===== 前月末 =====
        hist_month = t.history(period="3mo")
        month_close = hist_month["Close"].resample("M").last()
        prev_month = month_close.iloc[-2] if len(month_close) >= 2 else close.iloc[0]
